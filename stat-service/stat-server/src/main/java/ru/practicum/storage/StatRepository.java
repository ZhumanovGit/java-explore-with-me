...
public interface StatRepository extends JpaRepository<Stat, Long> {
    ...
    @Query("select s.app as app, s.uri as uri, count(s.ip) as hits " +
            ...
            "group by app, uri " +
            "order by hits desc")
    List<ViewStat> countHitsByAppAndUriForPeriod(@Param("start") @NonNull LocalDateTime start,
    ...
    @Query("select s.app as app, s.uri as uri, count(distinct s.ip) as hits " +
            ...
            "group by app, uri " +
            "order by hits desc")
    List<ViewStat> countUniqueHitsByAppAndUriForPeriod(@Param("start") @NonNull LocalDateTime start,
                                                       @Param("end") @NonNull LocalDateTime end,
                                                       @Param("uris") @Nullable List<String> uris);
}
